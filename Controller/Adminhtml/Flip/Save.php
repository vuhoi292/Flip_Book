...
namespace Book\Flip\Controller\Adminhtml\Flip;
...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            $model = $this->_objectManager->create('Book\Flip\Model\Flip')->load($id);
            if (!$model->getId() && $id) {
            ...
            if (isset($_FILES['upload']) && $_FILES['upload']['name'] != '') {
                $uploader = $this->_objectManager->get('Book\Flip\Model\Flip\Upload');
                $backgroundModel = $this->_objectManager->get('Book\Flip\Model\Flip\Image');
            ...
            if (isset($_FILES['upload']) && $_FILES['upload']['name'] != '') {
                $uploader = $this->_objectManager->get('Book\Flip\Model\Flip\Upload');
                $backgroundModel = $this->_objectManager->get('Book\Flip\Model\Flip\Image');
                $zipName = $uploader->uploadFileAndGetName('upload', $backgroundModel->getBaseDir(), $data);
                $data['upload'] = $_FILES['upload']['name'];
                $zipPath = substr($zipName, 0, strrpos($zipName, '/'));
            }else{
                $data['upload'] = $data['upload']['value'];
            }      

            $file = $this->directory_list->getPath('media').'/book'.$zipPath.'/'.$data['upload'];
            $path = $this->directory_list->getPath('media').'/book'.$zipPath;
            //$path = pathinfo(realpath($file), PATHINFO_DIRNAME);
            ...
            if ($res === TRUE) {
              ...
              $this->messageManager->addErrorMessage('Something went wrong while saving the Flip.');
               
            }*/
            

            //echo $data['upload']; exit;
